...
class Formal(Verification):
    ...
    def check(self, m: Module):
        ...
        self.assert_registers(m, A=a, B=b, X=LCat(
            x_hi, x_lo), SP=self.data.pre_sp+7, PC=LCat(ret_hi, ret_lo))
        self.assert_flags(m, H=ccs[Flags.H], I=ccs[Flags.I], N=ccs[Flags.N],
                          Z=ccs[Flags.Z], V=ccs[Flags.V], C=ccs[Flags.C])
